module Dag
  module Edges
    ...
    def build_edge(ancestor, descendant)
      ...
      conditions = conditions_for(source, sink)
      path = new(conditions)
      path.make_direct
      path
    end
    ...
    def find_edge(ancestor, descendant)
      source = self::EndPoint.from(ancestor)
      sink = self::EndPoint.from(descendant)
      where(conditions_for(source, sink).merge!({direct_column_name => true})).first
    end
    ...
    def longest_path_between(ancestor, descendant, path = [])
      ...
      ancestor.children.each do |child|
        ...
        elsif find_link(child, descendant)
          temp = path.clone
          temp << child
          temp = longest_path_between(child, descendant, temp)
          if temp.length > longest.length
            longest = temp
          end
        end
    ...
    def shortest_path_between(ancestor, descendant, path = [])
      ...
      ancestor.children.each do |child|
        if child == descendant
          ...
          temp = path.clone
          temp << child
          temp = shortest_path_between(child, descendant, temp)
          if shortest.blank? || temp.length < shortest.length
            shortest = temp
          end
        end
    ...
    def edge?(ancestor, descendant)
      !find_edge(ancestor, descendant).nil?
    end
    ...
    module EdgeInstanceMethods
      ...
      def perpetuate
        ...
        return true if do_not_perpetuate
      ...
      def links_to_source
        self.class.with_descendant_point(source)
      end

      # all links that start from the sink
      def links_from_sink
        self.class.with_ancestor_point(sink)
      end
      ...
      def wiring
        ...
        sink = self.sink
        above_sources = []
        links_to_source.each do |edge|
          above_sources << edge.source
        end
        below_sinks = []
        links_from_sink.each do |edge|
        ...
        below_sinks.each do |below_sink|
          ...
          push_associated_modification!(below_bridging_leg)
          above_bridging_legs.each do |above_bridging_leg|
            long_leg = rewire_crossing(above_bridging_leg, below_leg)
            push_associated_modification!(long_leg)
          end
        end
        above_bridging_legs.each do |above_bridging_leg|
          push_associated_modification!(above_bridging_leg)
        end
      end
    end
