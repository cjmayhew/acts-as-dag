module Dag
  module Polymorphic
    ...
    class EndPoint
      ...
      def matches?(other)
        return (id == other.id) && (type == other.type) if other.is_a?(EndPoint)

        (id == other.id) && (type == other.class.to_s)
      end

      # Factory Construction method that creates an EndPoint instance from a model
      def self.from_resource(resource)
        new(resource.id, resource.class.to_s)
      end
    ...
    class Source < EndPoint
      ...
      def self.from_edge(edge)
        new(edge.ancestor_id, edge.ancestor_type)
      end
    end
    ...
    class Sink < EndPoint
      ...
      def self.from_edge(edge)
        new(edge.descendant_id, edge.descendant_type)
      end
    end
